...
func joinedNonEmpty(values []string, separator string) string {
...
func buildPrintIQCreateQuotePayload(values orderFormValues, summary *campaignSummary) map[string]any {
	...
	setStringIfPresent(externalProduct, "ProductCode", values.StockCode)
	...
	setStringIfPresent(externalProduct, "Print", strings.Join([]string{strings.TrimSpace(values.ProcessFront), strings.TrimSpace(values.ProcessReverse)}, " "))
	setStringIfPresent(externalProduct, "Stock", values.StockCode)
	setStringIfPresent(externalProduct, "Finish", joinedNonEmpty(values.SelectedSectionOperations, ", "))
	setStringIfPresent(externalProduct, "Packing", joinedNonEmpty(values.SelectedJobOperations, ", "))
